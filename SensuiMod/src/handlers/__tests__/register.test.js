const RegisterHandler = require('../registerSurvey');
...
describe('RegisterHandler', () => {
    ...
    beforeAll(() => {
        ...
        sut = new RegisterHandler(ethereumMgr)
    })
    ...
    describe('handle', () => {
        ...
        test('handle no report', async () => {
            ...
            await sut.handle(event, {}, (err, res) => {
                expect(err).not.toBeNull()
                expect(err.code).toEqual(400)
                expect(err.message).toEqual('report parameter missing')
            })
        })

        test('handle report not json', async () => {
            let event = {
                body: JSON.stringify({
                    surveyId: "test",
                    report: "{}}"
            ...
            await sut.handle(event, {}, (err, res) => {
                expect(err).not.toBeNull()
                expect(err.code).toEqual(400)
                expect(err.message).toEqual('report is not valid JSON')
            })
        })
